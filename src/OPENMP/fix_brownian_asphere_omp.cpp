...
#include "fix_brownian_asphere_omp.h"
...
FixBrownianAsphereOMP::FixBrownianAsphereOMP(LAMMPS *lmp, int narg, char **arg) :
    FixBrownianBaseOMP(lmp, narg, arg), avec(nullptr)
{
...
void FixBrownianAsphereOMP::init()
{
...
void FixBrownianAsphereOMP::initial_integrate(int /*vflag */)
{
#if defined(_OPENMP)
#pragma omp parallel LMP_DEFAULT_NONE LMP_SHARED(dipole_flag,noise_flag)
#endif
  { // begin parallel region
    ...
    if (domain->dimension == 2) {
      if (dipole_flag) {
	if (!noise_flag) {
	  initial_integrate_templated<0, 0, 1, 1>(ifrom, ito, thr);
	} else if (gaussian_noise_flag) {
	  initial_integrate_templated<0, 1, 1, 1>(ifrom, ito, thr);
	} else {
	  initial_integrate_templated<1, 0, 1, 1>(ifrom, ito, thr);
	}
      } else {
	if (!noise_flag) {
	  initial_integrate_templated<0, 0, 0, 1>(ifrom, ito, thr);
	} else if (gaussian_noise_flag) {
	  initial_integrate_templated<0, 1, 0, 1>(ifrom, ito, thr);
	} else {
	  initial_integrate_templated<1, 0, 0, 1>(ifrom, ito, thr);
	}
      }
    } else {
      if (dipole_flag) {
	if (!noise_flag) {
	  initial_integrate_templated<0, 0, 1, 0>(ifrom, ito, thr);
	} else if (gaussian_noise_flag) {
	  initial_integrate_templated<0, 1, 1, 0>(ifrom, ito, thr);
	} else {
	  initial_integrate_templated<1, 0, 1, 0>(ifrom, ito, thr);
	}
      } else {
	if (!noise_flag) {
	  initial_integrate_templated<0, 0, 0, 0>(ifrom, ito, thr);
	} else if (gaussian_noise_flag) {
	  initial_integrate_templated<0, 1, 0, 0>(ifrom, ito, thr);
	} else {
	  initial_integrate_templated<1, 0, 0, 0>(ifrom, ito, thr);
	}
      }
    }
...
void FixBrownianAsphereOMP::initial_integrate_templated(int ifrom, int ito, ThrData * const thr)
{
